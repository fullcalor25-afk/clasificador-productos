...
export default async function handler(req, res) {
  ...
  logRequest('corrections', { method: req.method, query: req.query, body: req.body })
  ...
  if (req.method === 'DELETE') {
    ...
    if (all === 'true') {
      const r = await fetch(SUPABASE_URL + '/rest/v1/corrections?id=gt.0', {
        method: 'DELETE',
    ...
    if (id) {
      const r = await fetch(SUPABASE_URL + '/rest/v1/corrections?id=eq.' + id, {
        method: 'DELETE',
        headers: { ...h, Prefer: 'return=minimal' },
      })
      if (!r.ok) {
    ...
    if (codigo) {
      const r = await fetch(SUPABASE_URL + '/rest/v1/corrections?codigo=eq.' + encodeURIComponent(codigo), {
        method: 'DELETE',
        headers: { ...h, Prefer: 'return=minimal' },
      })
      if (!r.ok) {
