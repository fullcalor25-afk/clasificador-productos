...
// Structure: { "n1|n2": { label, rows_by_n3: { "n3": [row, ...] } } }
...
const BLANK_FORM = { id: null, nivel1: "", nivel2: "", nivel3: "", nivel4: "", keywords: "" };
...
export default function TnCategoriesView({ tnCategories = [], loading, onSave, onDelete }) {
  ...
  const Field = ({ label, field, placeholder }) => (
    <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
      <label style={{ fontSize: 11, fontWeight: 600, color: C.textMuted, textTransform: "uppercase" }}>{label}</label>
      <input
        value={form[field]}
              ...
              {isOpen && (
                <div style={{ padding: "8px 12px 12px", display: "flex", flexDirection: "column", gap: 6 }}>
                  {Object.entries(group.byN3).map(([n3key, rows]) => {
                    ...
                    const isN3Open = openN3[n3OpenKey] !== false; // default open
                            ...
                            {rows.map(row => (
                              ...
                              >
                                ...
                                {row.keywords && (
                                  <span
                                    style={{
                                      flex: 1, fontSize: 11, color: C.textDim,
                                ...
                                <div style={{ display: "flex", gap: 6, marginLeft: "auto", flexShrink: 0 }}>
                                  ...
                                  <BTN small danger onClick={() => { setDeleteTarget(row); setModal("delete"); }}>🗑</BTN>
                                </div>
                              </div>
                            ))}
                          </div>
                  ...
                  {Object.keys(group.byN3).length === 0 && (
                    <div style={{ color: C.textDim, fontSize: 12, padding: "4px 8px" }}>
                      Sin subgrupos todavía.
                    </div>
                  )}
                </div>
              )}
          ...
          <div style={{ background: C.surface, borderRadius: 16, border: `1px solid ${C.border}`, padding: 28, width: "100%", maxWidth: 500, maxHeight: "90vh", overflowY: "auto" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                <Field label="Nivel 1 *" field="nivel1" placeholder="Repuestos y Accesorios" />
                <Field label="Nivel 2" field="nivel2" placeholder="Calefaccion" />
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                <Field label="Nivel 3" field="nivel3" placeholder="Calderas" />
                <Field label="Nivel 4" field="nivel4" placeholder="Plaquetas" />
              </div>

              <div style={{ padding: "8px 12px", borderRadius: 8, background: `${C.accent}10`, fontSize: 12, color: C.accent }}>
                <strong>Ruta:</strong>{" "}
                {[form.nivel1, form.nivel2, form.nivel3, form.nivel4].filter(Boolean).join(" > ") || "(vacío)"}
              </div>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.55)", zIndex: 300, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
          <div style={{ background: C.surface, borderRadius: 14, border: `1px solid ${C.border}`, padding: 26, width: "100%", maxWidth: 400 }}>
            ...
            <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
              <BTN onClick={() => { setModal(null); setDeleteTarget(null); }}>Cancelar</BTN>
              <button
